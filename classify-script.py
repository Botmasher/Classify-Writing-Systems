...
def classify_script(symbols, script='This script', double_bit=False):
	'''
	Guess the classification of this script by counting the number of distinct characters it contains.
	Accuracy increases as the number of unique symbols in this sample approaches the total number
	of signs in the script.

	@param symbols 		:	a string containing characters in a single script
	@param script 		:	an optional string naming the script
	@param double_bit 	:	an optional Boolean to avoid double-counting double-bit characters
								/!\ ALWAYS SET FALSE if count_distinct decodes utf-8 chars
	-> script_type 		:	a string reporting the guessed classification of the script
	'''
